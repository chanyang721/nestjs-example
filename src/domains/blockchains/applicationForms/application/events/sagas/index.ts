import {
  ApproveApplicationFormCommandImplement,
} from '@/blockchains/applicationForms/application/commands/handlers/approve.application.form.command';
import {
  ...
  dragonKilled = ( events$: Observable<any> ): Observable<ICommand> => {
    return events$
      .pipe(
        ...
        delay(1000),
        map( ( event ) =>
          new ApproveApplicationFormCommandImplement( event.applicationFormId, this.statusMap.get( 'status key' ) ) ),
      );
  };
}